...
async function additiveEncryption(text: string, key: string) {
    const response = await fetch(API_URL + "/additive/encrypt", {
        method: "POST",
        headers: {
...
async function additiveDecryption(text: string, key: string) {
    const response = await fetch(API_URL + "/additive/decrypt", {
        method: "POST",
        headers: {
            "Content-Type": "application/json",
        },
...
export default function Additive() {
    return (
        <div className="my-20">
            <h1 className="text-2xl mb-2">Additve Cipher</h1>
            <div className="flex justify-between">
                <CipherBox
                    type="encrypt"
                    fn={additiveEncryption}
                />
                <CipherBox
                    type="decrypt"
                    fn={additiveDecryption}
                />
            </div>
        </div>
    );
